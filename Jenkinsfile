node {
  ...
  stage('Build Docker image') {
    ...
    stage('connection') {   
		    ...
		    sh 'ssh ubuntu@ec2-34-238-156-135.compute-1.amazonaws.com if [  "$(docker ps -q -f name=sanyam)" ]; then docker kill sanyam docker rm sanyam fi docker pull sanyambatra13/jenkins-webapp:ver1 docker run --name sanyam -d -p 8888:8888 sanyambatra13/jenkins-webapp:ver1'
